...
def parse_sofr(data):
    ...
    return {
        ...
        "history": [{"date": r["effectiveDate"], "rate": r["percentRate"]} for r in rates[:5]]
    }
...
def parse_onrrp(data):
    ...
    history = [{"date": o["operationDate"], "amount_bn": round(o["totalAmtAccepted"]/1e9, 3)} for o in ops[:5]]
    return {
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {executor.submit(fetch, url, label): label for label, url in urls.items()}
