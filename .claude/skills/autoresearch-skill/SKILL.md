...
name: autoresearch
description: |
  Use when: user asks to optimize a Claude Code skill, improve skill performance, run evals, or says "optimize this skill" / "improve skill" / "autoresearch" / "run evals"
  Don't use when: user wants to optimize a plain markdown prompt document (use autotune instead). Counter-example: "optimize this CLAUDE.md" / "improve my prompt file"
---
...
**IMPORTANT: If you detect that the user has switched between different skills within the same conversation turn, remind them to start a new session to avoid context contamination.**
...
2. Add the new directory to `.gitignore` so experiment artifacts are not committed:
   ```
   # Autoresearch output directories (per-skill experiment results)
