...
### Text/copy prompts (newsletters, tweets, emails, landing pages)
...
### Visual/design prompts (diagrams, images, slides)
...
### Code/technical prompts (code generation, configs, scripts)
...
### Document prompts (proposals, reports, decks)
...
### 1. Too many evals
More than 6 evals and the prompt starts gaming them — it optimizes for passing the test instead of producing good output. Like a student who memorizes answers without understanding the material.

**Fix:** Pick the 3-6 checks that matter most. If everything passes those, the output is probably good.

### 2. Too narrow/rigid
"Must contain exactly 3 bullet points" or "Must use the word 'because' at least twice" — these create prompts that technically pass but produce weird, stilted output.
...
2. **Could a prompt game this eval without actually improving?** If yes, the eval is too narrow. Broaden it.
