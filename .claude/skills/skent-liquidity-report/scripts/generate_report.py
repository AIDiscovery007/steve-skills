...
def fmt_value(value, digits=2, prefix="", suffix=""):
...
def fmt_change(change, change_pct):
    if not is_number(change) or not is_number(change_pct):
        return "N/A"
...
def assess_sofr(sofr):
    ...
    if has_error(sofr) or not is_number(rate):
        return "⚪", "SOFR 数据缺失"
...
def assess_move(move):
    ...
    if has_error(move) or not is_number(price):
        return "⚪", "MOVE 数据缺失"
...
def assess_usdjpy(usdjpy):
    ...
    if has_error(usdjpy) or not is_number(price) or not is_number(change_pct):
        return "⚪", "USDJPY 数据缺失"
...
def assess_hyg(hyg):
    ...
    if has_error(hyg) or not is_number(price) or not is_number(w52_high):
        return "⚪", "HYG 数据缺失"
...
def assess_onrrp(onrrp):
    ...
    if has_error(onrrp) or not is_number(amount):
        return "⚪", "ON RRP 数据缺失"
...
def assess_gold(gold):
    ...
    if has_error(gold) or not is_number(price) or not is_number(w52_high):
        return "⚪", "黄金数据缺失"
...
def assess_dxy(dxy):
    ...
    if (
        has_error(dxy)
        or not is_number(price)
        or not is_number(w52_high)
        or not is_number(w52_low)
    ):
        return "⚪", "美元指数数据缺失"
...
def assess_tyx(tyx):
    ...
    if has_error(tyx) or not is_number(price) or not is_number(w52_high):
        return "⚪", "TYX 数据缺失"
...
def generate(data: dict) -> str:
    ...
    overall = overall_assessment(
        [sig_sofr, sig_move, sig_usd, sig_hyg, sig_dxy, sig_tip]
    )
    ...
    lines.append(
        f"- 最新余额：**{fmt_value(onrrp.get('amount_bn'), digits=2, prefix='$', suffix='B')}**（{onrrp.get('date', 'N/A')}）"
    )
    lines.append(f"- {note_rrp}")
    lines.append(
        f"- *注：净流动性 = 美联储总资产 - TGA - ON RRP，ON RRP 余额低 → 流动性已流入市场*"
    )
    lines.append(f"")
    lines.append(f"---")
    lines.append(f"")
    ...
    lines.append(f"")
    lines.append(
        f"{sig_sofr} **SOFR：{sofr.get('rate', 'N/A')}%**（{sofr.get('date', 'N/A')}）"
    )
    lines.append(
        f"- 成交量：${sofr.get('vol_bn', 'N/A')}B | P25: {sofr.get('p25', 'N/A')}% | P75: {sofr.get('p75', 'N/A')}%"
    )
    lines.append(f"- {note_sofr}")
    if sofr.get("history"):
        lines.append(
            f"- 近5日走势：{' → '.join([str(r['rate']) + '%' for r in sofr['history'][:5]])}"
        )
    lines.append(f"")
    lines.append(f"---")
    lines.append(f"")
    ...
    lines.append(f"")
    lines.append(
        f"{sig_move} **MOVE：{move.get('price', 'N/A')}**（今日 {fmt_change(move.get('change'), move.get('change_pct'))}）"
    )
    lines.append(
        f"- 52周区间：{move.get('52w_low', 'N/A')} - {move.get('52w_high', 'N/A')}"
    )
    ...
    lines.append(f"")
    lines.append(
        f"{sig_usd} **USDJPY：{usdjpy.get('price', 'N/A')}**（今日 {fmt_change(usdjpy.get('change'), usdjpy.get('change_pct'))}）"
    )
    lines.append(
        f"- 52周区间：{usdjpy.get('52w_low', 'N/A')} - {usdjpy.get('52w_high', 'N/A')}"
    )
    ...
    lines.append(f"")
    lines.append(
        f"{sig_hyg} **HYG：${hyg.get('price', 'N/A')}**（今日 {fmt_change(hyg.get('change'), hyg.get('change_pct'))}）"
    )
    lines.append(
        f"- 52周区间：${hyg.get('52w_low', 'N/A')} - ${hyg.get('52w_high', 'N/A')}"
    )
    ...
    lines.append(f"")
    lines.append(
        f"{sig_dxy} **美元指数（DXY）：{fmt_value(dxy.get('price'), digits=3)}**（今日 {fmt_change(dxy.get('change'), dxy.get('change_pct'))}）"
    )
    lines.append(
        f"- 52周区间：{dxy.get('52w_low', 'N/A')} - {dxy.get('52w_high', 'N/A')}"
    )
    lines.append(f"- {note_dxy}")
    lines.append(f"- *美元走强 → 黄金承压；美元走弱 → 黄金受益*")
    lines.append(f"")
    lines.append(
        f"{sig_tip} **TIPS ETF（TIP）：{fmt_value(tip.get('price'), digits=2, prefix='$')}**（今日 {fmt_change(tip.get('change'), tip.get('change_pct'))}）"
    )
    lines.append(
        f"- 52周区间：${tip.get('52w_low', 'N/A')} - ${tip.get('52w_high', 'N/A')}"
    )
    lines.append(f"- {note_tip}")
    lines.append(f"- *TIP 价格与实际利率反向；实际利率降 → 黄金最强驱动*")
    lines.append(f"")
    lines.append(
        f"{sig_tyx} **30年国债收益率（TYX）：{fmt_value(tyx.get('price'), digits=3, suffix='%')}**（今日 {fmt_change(tyx.get('change'), tyx.get('change_pct'))}）"
    )
    lines.append(
        f"- 52周区间：{tyx.get('52w_low', 'N/A')}% - {tyx.get('52w_high', 'N/A')}%"
    )
    ...
    lines.append(f"")
    lines.append(
        f"**S&P 500：{spx.get('price', 'N/A')}**（今日 {fmt_change(spx.get('change'), spx.get('change_pct'))}）"
    )
    lines.append(
        f"- 52周区间：{spx.get('52w_low', 'N/A')} - {spx.get('52w_high', 'N/A')}"
    )
    lines.append(f"")
    lines.append(
        f"{sig_gold} **黄金（GC=F）：{fmt_value(gold.get('price'), digits=1, prefix='$')}**（今日 {fmt_change(gold.get('change'), gold.get('change_pct'))}）"
    )
    lines.append(f"- {note_gold}")
    lines.append(f"")
    lines.append(
        f"**白银（SI=F）：${silver.get('price', 'N/A')}**（今日 {fmt_change(silver.get('change'), silver.get('change_pct'))}）"
    )
    lines.append(
        f"- 52周区间：${silver.get('52w_low', 'N/A')} - ${silver.get('52w_high', 'N/A')}"
    )
    lines.append(f"")
    lines.append(f"---")
    lines.append(f"")
    ...
    lines.append(f"```")
    lines.append(
        f"净流动性水位  {sig_rrp}  ON RRP {fmt_value(onrrp.get('amount_bn'), digits=1, prefix='$', suffix='B')}"
    )
    lines.append(f"      ↓")
